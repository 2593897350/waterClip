...
grep -F 'ARG NPM_BOOTSTRAP_REGISTRY=' web/Dockerfile >/dev/null
grep -F 'ARG NPM_STRICT_SSL=' web/Dockerfile >/dev/null
grep -F 'npm config set strict-ssl "$NPM_STRICT_SSL"' web/Dockerfile >/dev/null
grep -F 'npm install -g pnpm --registry "$NPM_BOOTSTRAP_REGISTRY"' web/Dockerfile >/dev/null
...
grep -F 'NPM_BOOTSTRAP_REGISTRY: ${NPM_BOOTSTRAP_REGISTRY:-https://registry.npmjs.org/}' docker-compose.yml >/dev/null
...
grep -F 'NPM_BOOTSTRAP_REGISTRY=https://registry.npmjs.org/' .env.production.example >/dev/null
